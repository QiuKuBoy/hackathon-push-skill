...
多维表格的价值在于可筛选/排序：参赛者可按「来源分类=金融」「状态=即将截止」「剩余天数」快速定位高价值赛事。
...
推送飞书为可选项，`push_enabled` 默认 `true`；每次运行可用 `--push` / `--no-push` 临时覆盖（优先级 `--no-push` > `--push` > `push_enabled`），详见 SKILL.md 第四步。多维表同步不受此开关影响，始终更新。
